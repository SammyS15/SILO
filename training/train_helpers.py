...
def load_optimizer(config, diffusion_extractor, aggregation_network):
    ...
    # ]
    parameter_groups = [{"params": aggregation_network.parameters(), "lr": config["lr"]}]
...
def get_hyperfeats(diffusion_extractor, aggregation_network, imgs, eval_mode=False,is_latents=False,sigma_emb=None,kernel=None,make_w_t=None,target=None,cond_inpaint=None):
    with torch.inference_mode():
        feats, latents = diffusion_extractor.forward(imgs, eval_mode=eval_mode,is_latents=is_latents)
        b, s, l, w, h = feats.shape
        if make_w_t and target is not None:
            diffusion_extractor.skip_t_choosing = True
            _, latents_w_t = diffusion_extractor.forward(target, eval_mode=eval_mode,is_latents=is_latents)
            diffusion_extractor.skip_t_choosing = False

    diffusion_hyperfeats = aggregation_network(feats.clone().float().view((b, -1, w, h)), diffusion_extractor.emb.clone(),latents[1].clone(),sigma_emb=sigma_emb.clone(),kernel=kernel,cond_inpaint=cond_inpaint)
    if make_w_t and target is not None:
        return diffusion_hyperfeats , latents, latents_w_t
    return diffusion_hyperfeats , latents
...
def log_aggregation_network(aggregation_network, config):
    mixing_weights = torch.nn.functional.softmax(aggregation_network.mixing_weights, dim=0)
