...
def center_sq_bbox(img, mask_shape, image_size=256):
    """Generate a square mask in the center of the image for inpainting
    """
    # B, C, H, W = img.shape
    B, C, H, W = 1, 3, image_size, image_size
    h, w = mask_shape

    # Calculate the top-left corner to center the box
    t = (H - h) // 2
    l = (W - w) // 2

    # Create mask
    # mask = torch.ones([B, C, H, W], device=img.device)
    mask = torch.ones([B, C, H, W], device="cuda")
    mask[..., t:t+h, l:l+w] = 0

    return mask, t, t+h, l, l+w

def random_fixed_sq_mask(img, image_size=256, margin=(16, 16)):
    """Generate random square masks with a fixed size of 256x256 for inpainting
    
    Args:
        img: Input image tensor of shape [B, C, H, W]
        image_size: Size of the square image (default: 256)
        margin: Margins to keep from the image borders (height_margin, width_margin)
    
    Returns:
        mask: A tensor of shape [B, C, H, W] with values 0 in the masked regions and 1 elsewhere
        coordinates: List of tuples (t, b, l, r) for each image in the batch
    """
    B, C, H, W = img.shape
    h, w = 256, 256  # Fixed mask size as specified
    margin_height, margin_width = margin
    
    # Check if there's enough space for the mask given the image size and margins
    maxt = image_size - margin_height - h
    maxl = image_size - margin_width - w
    
    # Initialize mask with ones (no masking)
    mask = torch.ones([B, C, H, W], device=img.device)
    coordinates = []
    
    # Generate different mask for each image in the batch
    for b in range(B):
        # Generate random position for this image
        t = np.random.randint(margin_height, maxt)
        l = np.random.randint(margin_width, maxl)
        
        # Apply mask to this specific image
        mask[b, :, t:t+h, l:l+w] = 0
        
        # Store coordinates
        coordinates.append((t, t+h, l, l+w))
    
    return mask, coordinates

class mask_generator:
    def __init__(self, mask_type, mask_len_range=None, mask_prob_range=None,
                 image_size=256, margin=(16, 16), random = False):
        """
    ...
    def _retrieve_box(self, img):
        ...
        if self.random:
            mask, _ = random_fixed_sq_mask(img,
