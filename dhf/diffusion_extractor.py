...
class DiffusionExtractor:
    ...
    def forward(self, images, guidance_scale=-1, preview_mode=False, eval_mode=False,is_latents=False):
        ...
        if eval_mode or self.eval_mode:
            ...
            if hasattr(self,"manual_t") and (1<=self.manual_t<=999):
                self.save_timestep = [self.num_timesteps - self.manual_t]
            elif hasattr(self,"skip_t_choosing") and self.skip_t_choosing: 
                pass
            else:
                self.save_timestep = [np.random.choice(range(1, self.num_timesteps))]
                # self.save_timestep = [np.random.choice(range(1, 300))]
            # self.save_timestep = [np.random.choice(range(1, self.num_timesteps))]
            # print(f"{self.num_timesteps=}")
            noise_timestep = self.num_timesteps - 1 - self.save_timestep[0]
            noise = torch.randn_like(latents)
            # print(f"{self.save_timestep=}")
            # print(f"{noise_timestep=}")
        ...
        # print(f"{save_timestep=} {save_timestep+1=}")
